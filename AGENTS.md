...
| `npm run build` | ✅ يشتغل | **exit 0** | ❌ **ليست بوابة للمنفّذ** — انظر التحذير أدناه |
...
### تحذير `npm run build` — لا تشغّله كبوابة

يعطي exit 0، لكن:
1. **يقرأ Firestore الإنتاج وقت البناء.** `app/store/page.tsx:8-14` يستدعي `getStores()` داخل مكوّن سيرفر يُبنى prerendered؛ البناء ينفّذ قراءات حيّة على قاعدة البيانات الحقيقية (قراءة فقط، ملفوفة في `try/catch`).
2. **يحتاج `.env.local`.** ولا يجوز للمنفّذ قراءته (§3). CI نفسه لا يشغّل `build` أبدًا لهذا السبب بالنص: البوابة الصلبة اختيرت `tsc` لأنها *«لا تحتاج أي أسرار/متغيّرات بيئة»* (`.github/workflows/ci.yml`).
3. ينتج `.next/` و `tsconfig.tsbuildinfo` — ضجيج لا يخصّ الـ diff.

⇒ إن أردت التأكد من سلامة الأنواع، `npx tsc --noEmit` هو الأمر الصحيح، لا `build`.
...
- **ممنوع أي أمر يمسّ قاعدة بيانات أو خدمة حقيقية.** لا يوجد Firebase emulator في هذا المشروع (`firebase.json` فيه `firestore` و `storage` فقط، ولا ذكر لـ emulator في أي ملف) ⇒ **أي اتصال بـ Firestore هو اتصال بالإنتاج**. المحظور تحديدًا:
  ...
  - `npm run dev` / `npm start` / `npm run preview` / `npm run build`
